...
class Encoder_Decoder_Residual(nn.Module):
    def __init__(self,Discriminator_compare, rs_blocks=8):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(in_channels=1, out_channels=64, kernel_size=9, padding=0),
...
#             nn.ReLU(True)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReflectionPad2d(3),
            nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=9, padding=3),
        )
        ...
        self.decoder_down = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.ReflectionPad2d(1),
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=1, padding=4),

            nn.InstanceNorm2d(128, track_running_stats=False),
            nn.LeakyReLU(0.1),

            nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=6, stride=1, padding=3),
            nn.InstanceNorm2d(64, track_running_stats=False),
            nn.LeakyReLU(0.1),
    ...
    def forward(self, x):
        inputt = x
        x_down = F.interpolate(inputt, scale_factor=0.5)
        x_down = self.encoder_down(x_down)
        x_down = self.head(x_down)
        out_down = self.decoder_down(x_down)
        x = self.encoder(inputt)
...
#             nn.Upsample(scale_factor=2, mode='bilinear'),
#             nn.ReflectionPad2d(1),
#             nn.ConvTranspose2d(256, 128, kernel_size=4, stride=1, padding=3),
...
#         x_down = self.head(x_down)
#         out_down = self.decoder_down(x_down)
#         x_down = F.interpolate(x_down, scale_factor=2)
#         x = self.encoder(inputt)
# #         print(out_down.size())
# #         print(x_down.size())
#         x = torch.cat((x, x_down),1)
